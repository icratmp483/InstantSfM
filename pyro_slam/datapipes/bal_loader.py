...
def read_bal_data(file_name: str, use_quat=False) -> dict:
    ...
    with open(file_name, "r") as file:
        ...
        points_2d = torch.empty((n_observations, 2), dtype=torch.float32)
        ...
        camera_params = torch.empty(n_cameras * 9, dtype=torch.float32)
        for i in range(n_cameras * 9):
            camera_params[i] = float(file.readline())
        camera_params = camera_params.reshape((n_cameras, -1))

        points_3d = torch.empty(n_points * 3, dtype=torch.float32)
...
def _test():
    ...
    for i in dp:
        ...
        assert torch.float32 == points.dtype == pixels.dtype == camera_params.dtype, "dtype not float32."
    ...
    for dataset in ALL_DATASETS:
        ...
        for i in dp:
            ...
            assert torch.float32 == camera_params.dtype, "dtype not float32."
