...
class TrackEngine:
    ...
    def TrackCollection(self, TRACK_ESTABLISHMENT_OPTIONS):
        ...
        tracks = {track_id: np.concatenate([np.array(list(correspondences.keys())), -np.array(list(correspondences.values()))[:, None]], axis=-1) for track_id, correspondences in track_map.items()}
        discarded_counter = 0
        for track_id in list(tracks.keys()):
            ...
            for image_id, feature_id, _ in tracks[track_id]:
                ...
                if image_id not in image_id_set:
                    image_id_set[image_id] = np.array(image_feature).reshape(1, 2)
                else:
                    image_id_set[image_id] = np.concatenate((image_id_set[image_id], np.array(image_feature).reshape(1, 2)), axis=0)
                    image_features_array = np.array(image_id_set[image_id])
                    distances = np.linalg.norm(image_features_array - image_feature, axis=1)
